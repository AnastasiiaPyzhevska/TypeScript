...
export function logFirstAvailable(book: readonly Book[] = getAllBooks()): void {
    console.dir(`Number of books: ${book.length}`);

    const title = book.find(book=> book.available)?.title;
    console.dir(`First available book: ${title}`);
}
...
export function logBookTitles(titles: Array<string>): void {
    titles.forEach(title=> console.dir(title));
}
...
export function createCustomer( name: string, age?: number, city?: string): void {
    console.dir( `Customer name: ${name}`);
    if (age) {
        console.dir( `Customer age: ${age}`);
    }
    if (city) {
        console.dir( `Customer city: ${city}`);
    }
};
...
export function printBook( book: Book): void {
    console.dir(`${book.title} by ${book.author} `);
};
...
export function logCategorySearch(err: Error, titles: string[] | null): void {
    if (err) {
